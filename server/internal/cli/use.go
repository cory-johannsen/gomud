...
func (u *UseHandler) useObject(objectName string, target *string) (string, error) {
	...
	result, err := object.Interact(state, &player.Character, target,
		func(obj domain.InteractiveObject, state domain.GameState, user *domain.Character, target *string) {
			obj.SetBusy(true)
		},
		func(obj domain.InteractiveObject, state domain.GameState, user *domain.Character, target *string, result string, err error) {
			obj.SetBusy(false)
		},
	)
	if err != nil {
